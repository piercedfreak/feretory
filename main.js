...
function recordLearningFeedback(item, vote) {
  ...
  // 🔑 stable key per item (prevents spam voting)
  const voteKey = item.dedupeKey || item.link || `${item.pluginId}|${item.title}`;

  // 🚫 block duplicate votes
  if ((learning.feedback || []).some(f => f.voteKey === voteKey)) {
    return {
      ok: false,
...
ipcMain.handle('learning:feedback', async (_event, item, vote) => {
  return recordLearningFeedback(item, vote);
	// prevent duplicate voting on same item
 const voteKey = item.dedupeKey || item.link || `${item.pluginId}|${item.title}`;
